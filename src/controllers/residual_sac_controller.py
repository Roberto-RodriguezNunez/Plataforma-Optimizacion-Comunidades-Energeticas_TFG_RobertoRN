...
========================================================================
Subclase de ResidualControllerBase que carga el modelo entrenado (.zip) y las
stats de VecNormalize (.pkl), e implementa la inferencia con torch (SB3.predict).
Toda la lógica residual (MPC + obs + normalización + combinación) vive en la base.

Sirve para CUALQUIER off-policy continuo entrenado sobre ResidualEnv (SAC, TD3,
DDPG): todos exponen `model.predict(obs, deterministic=True) -> (delta, _)`. El
nombre "SAC" es histórico (era el algoritmo principal).

El backend desplegable sin torch es OnnxResidualController (production/onnx_inference.py).
"""
...
class ResidualSACController(ResidualControllerBase):
    """
    Controlador residual con inferencia torch (SB3). Backend de evaluación
    (justo tras entrenar se tiene el .zip; la versión desplegable es la ONNX).
