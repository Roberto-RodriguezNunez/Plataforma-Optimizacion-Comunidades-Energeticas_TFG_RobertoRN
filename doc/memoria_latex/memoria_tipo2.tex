...
%  Centro : ESEI — Universidade de Vigo  |  Curso 2025/2026
%
...
\usepackage{csquotes} % comillas conforme al idioma; requerido por biblatex+babel
...
% Sin \flushbottom: con twoside, muchas figuras [H] y parskip elástico,
% LaTeX estiraba el espacio vertical para cuadrar cada página y dejaba
% huecos irregulares entre títulos y texto.
\raggedbottom
...
% Espaciado fijo alrededor de los títulos de sección: sin esto, el
% pegamento elástico de parskip inflaba de forma visible el hueco entre
% el título y su texto según la página.
\titlespacing*{\section}{0pt}{3.25ex plus 1ex minus .2ex}{1.5ex plus .2ex}
\titlespacing*{\subsection}{0pt}{2.75ex plus .8ex minus .2ex}{1.25ex plus .2ex}
\titlespacing*{\subsubsection}{0pt}{2.25ex plus .6ex minus .2ex}{1ex plus .2ex}
...
% Lenguaje YAML para el apéndice de configuración. El fichero real contiene
% caracteres Unicode (griegas, flechas, superíndices) que listings no acepta
% con inputenc: se mapean vía literate.
\lstdefinelanguage{yaml}{
  ...
  literate={á}{{\'a}}1 {é}{{\'e}}1 {í}{{\'i}}1 {ó}{{\'o}}1 {ú}{{\'u}}1
           ...
           {×}{{$\times$}}1 {→}{{$\to$}}1 {η}{{$\eta$}}1 {ρ}{{$\rho$}}1
           {σ}{{$\sigma$}}1 {±}{{$\pm$}}1 {²}{{$^{2}$}}1 {⁴}{{$^{4}$}}1
           {–}{{--}}1 {—}{{---}}1 {·}{{$\cdot$}}1 {Í}{{\'I}}1
}

% Diagrama de Gantt (planificado vs. real)
\usepackage{pgfgantt}
\definecolor{ganttreal}{RGB}{31,119,180}
\definecolor{ganttplan}{RGB}{200,200,200}

\usepackage[hidelinks, breaklinks=true]{hyperref}
\usepackage{pdfpages}
\usepackage{cleveref}
% Bibliografía en APA 7 (biblatex-apa + biber), con localización española.
% natbib=true mantiene los \citep/\citet existentes en el cuerpo.
\usepackage[backend=biber, style=apa, natbib=true]{biblatex}
\DeclareLanguageMapping{spanish}{spanish-apa}
\addbibresource{referencias.bib}
% Permitir cortes de línea dentro de las URLs de la bibliografía
...
\hypersetup{
  ...
  pdfsubject={TFG Tipo II — ESEI, Universidade de Vigo, 2026}
}
...
Tras un proceso documentado de exploración con las familias \acrshort{dqn},
\acrshort{ppo}, \acrshort{td3}, \acrshort{ddpg} y \acrshort{sac}, se diseña e implementa el
controlador Residual \acrshort{sac} sobre un \acrshort{mpc} base, arquitectura
híbrida que resuelve los problemas de arranque en frío y cuantización que
impidieron a los candidatos anteriores superar el \textit{baseline} competitivo
(\acrshort{mpc} con pronóstico realista, $+43{,}39$\,€/semana). En la suite de
validación final (tres semillas de entrenamiento, diez semillas de ruido y 50
semanas de evaluación no vistas), el controlador alcanza $+44{,}05$\,€/semana,
una mejora de $+0{,}66$\,€/semana estadísticamente significativa (test de
Wilcoxon pareado, $p<0{,}001$) sobre el \acrshort{mpc} realista.
...
al caso español (el marco de autoconsumo del RD 244/2019 o, como en este
trabajo, la cooperativa con un único punto de suministro cuya factura es el
balance horario agregado) ni resultan
...
\textbf{Modalidad adoptada.} Este trabajo se acoge al marco de autoconsumo del
RD~244/2019 y el RDL~7/2025, pero en la modalidad concreta de \textbf{cooperativa
con un único punto de suministro (CUPS)}: las quince viviendas se presentan ante
el sistema eléctrico a través de un solo contador frontera, por lo que la
liquidación económica se realiza sobre el \textbf{balance horario agregado} de la
comunidad (y no vivienda a vivienda mediante coeficientes de reparto). Ese balance
neto hora a hora es, exactamente, la factura eléctrica que optimiza el controlador.
La justificación de esta elección, sus condiciones de aplicabilidad y su encaje
jurídico se detallan en la Sección~\ref{sec:marco_regulatorio}.

La variabilidad del precio PVPC, que en el periodo analizado (2021--2023)
alcanzó un máximo horario de \num{0,95}\,€/kWh (marzo de 2022, crisis
energética) frente a valores cercanos a cero en horas valle, ofrece un margen
de arbitraje que solo un motor de decisión con capacidad predictiva puede
explotar. El
...
\begin{enumerate}[label=\textbf{OE\arabic*.}]
  ...
  \item Diseñar e implementar el controlador Residual \acrshort{sac} con
    ...
    experimentalmente que supera al \textit{baseline} competitivo mediante una
    suite estadística multisemilla que lo contrasta con las familias
    alternativas (DQN, PPO discreto y continuo, SAC puro, y TD3 y DDPG
    residuales).
...
sobre energías renovables y mercado interior de la
electricidad~\citep{Dir2018_2001, Dir2019_944}, se regula
...
subcontadores privados, sin reventa de energía conforme a la Ley 24/2013 del
Sector Eléctrico \citep{Ley24_2013}), no una
...
Distribución Cerradas, cuyo procedimiento de autorización desarrolla el Real
Decreto 314/2023 \citep{RD314_2023}, están reservadas a zonas industriales,
comerciales o de servicios compartidos (solo admiten consumo residencial como
excepción marginal y ligada a la industria, manteniendo además cada consumidor
su comercializadora), por lo que el escenario aquí adoptado se sostiene
jurídicamente como \emph{red interior de un único consumidor} (la cooperativa),
no como red de distribución cerrada. Su viabilidad económica se apoya en los
programas de incentivos a comunidades energéticas con almacenamiento (Programa
CE~IMPLEMENTA del IDAE, Orden TED/764/2024 \citep{TED764_2024}), que exigen
sistema de almacenamiento de forma obligatoria.
...
ESIOS~\citep{ESIOS}) es sistemáticamente inferior al de compra (PVPC,
indicador 1001), que
...
muestra un panorama matizado: la literatura encuentra que el MPC mantiene un
rendimiento ligeramente superior y se transfiere con más suavidad entre
escenarios \citep{Zhan2023}. En este dominio, un MPC determinista
correctamente sintonizado captura ya la mayor parte del valor económico
disponible mediante arbitraje diario y emparejamiento de picos de precio, y el
margen restante queda acotado por el coste de la incertidumbre del pronóstico.
En este trabajo ese margen se cuantifica de forma explícita para el caso de
estudio (la brecha entre el MPC con previsión perfecta y con previsión
realista resulta ser del 15{,}7\,\%; Sección~\ref{sub:mpc}), y la expectativa
realista para un esquema híbrido es recuperar solo una fracción de él. Este margen acotado, lejos de invalidar el enfoque, define con
...
el ámbito comercial como en proyectos de investigación: existen sistemas \acrshort{ems}
...
almacenamiento individual o industrial (donde la batería responde a un único
punto de consumo) y no a la batería compartida de una comunidad, en la que un
único activo debe operarse para el balance conjunto de todos los participantes
(balance que, en el modelo de cooperativa con un único punto de suministro
aquí adoptado, constituye además la factura eléctrica oficial;
Sección~\ref{sec:marco_regulatorio}). En tercer
...
La solución propuesta es la plataforma LeaLink, cuya arquitectura se organiza en
ocho capas funcionales repartidas en dos bloques: seis capas de investigación y
\textit{benchmarking} (Figura~\ref{fig:arquitectura}) y dos capas de producción
(Figura~\ref{fig:arquitectura_prod}). La comunidad modelada comprende quince
viviendas con perfil de consumo residencial 2.0TD, una instalación fotovoltaica
compartida de \SI{42}{kWp} y una batería comunitaria de tecnología
\acrshort{lfp} con capacidad de \SI{80}{kWh} e inversor bidireccional de
\SI{50}{kW}.

En el bloque de investigación, la \textbf{capa de datos y configuración}
construye y mantiene el dataset de simulación a partir de fuentes oficiales y
centraliza los parámetros del sistema; la \textbf{capa de simulación
físico-económica} implementa el gemelo digital de la comunidad; la \textbf{capa
de entornos de aprendizaje por refuerzo} envuelve ese simulador en interfaces
Gymnasium~\citep{Gymnasium2023}; la \textbf{capa de entrenamiento multisemilla}
optimiza los agentes sobre la partición de entrenamiento; la \textbf{capa de
controladores y agentes} reúne la librería común de políticas (heurístico, MPC y
controladores residuales) compartida entre investigación y producción; y la
\textbf{capa de evaluación comparativa} contrasta todos los controladores sobre
el conjunto \textit{held-out} con significancia estadística.

En el bloque de producción, la \textbf{capa de producción \textit{edge}}
gestiona el ciclo de vida del modelo entrenado y su exportación a formato
\acrshort{onnx} para la inferencia ligera en hardware de borde, y la \textbf{capa
SaaS} expone los resultados a los usuarios finales mediante una interfaz web
diferenciada por rol, con persistencia de los perfiles e histórico de la
comunidad.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{img/arquitectura_investigacion.png}
  \caption{Arquitectura de la plataforma, parte 1: capas de investigación y
  \textit{benchmarking} (datos, simulación, entornos, entrenamiento
  multisemilla, controladores y evaluación comparativa). La parte de
  producción (\textit{edge} y SaaS) se muestra en la
  Figura~\ref{fig:arquitectura_prod}.}
...
experimentación estructurado en dos fases: una fase de \textbf{exploración}
(establecimiento de cotas de rendimiento mediante MPC, experimentación con
candidatos (DQN, PPO) con análisis de limitaciones, y diseño del controlador
final (Residual SAC) que resuelve esas limitaciones) y una fase de
\textbf{validación} estadística, en la que el controlador propuesto se
confirma con tres semillas de entrenamiento y contraste de significancia
frente a las familias alternativas completas: DQN, PPO discreto, PPO continuo,
SAC puro y las variantes residuales con TD3 y DDPG
(Sección~\ref{sub:suite}). Este protocolo garantiza que la selección del
algoritmo está basada en evidencia experimental y no en preferencias a priori.
...
Proceso de Decisión de Markov (\acrshort{mdp}) definido por la tupla
$(\mathcal{S}, \mathcal{A}, P, R, \gamma)$~\citep{Sutton2018}.
...
política de referencia IDLE: el escenario contrafactual con la batería
desconectada (los cuatro flujos a cero), en el que la comunidad opera solo con
sus paneles y la red, autoconsumiendo lo que puede, comprando el déficit y
vertiendo el excedente. Su beneficio marginal es cero por definición, de modo
que toda cifra en €/semana de este trabajo mide el dinero que la gestión de la
batería añade sobre ese escenario sin almacenamiento:
\begin{equation}
  r_h = E_{\text{vend},h}\,p_v^h - E_{\text{comp},h}\,p_c^h - c_{\text{deg},h}
  \label{eq:recompensa}
\end{equation}

El factor de descuento $\gamma = 0{,}99$, valor canónico en la práctica del RL
profundo, establece un horizonte efectivo de $1/(1-\gamma) = 100$ pasos,
equivalente a unos cuatro días dentro de los episodios semanales de 168 horas.
La elección es deliberadamente mayor que las 24 horas del MPC: garantiza que
el agente pondere las consecuencias de sus decisiones sobre el estado de carga
más allá del horizonte de planificación, valorando el efecto de cargar o
descargar la batería sobre las jornadas siguientes (que el LP del MPC, por
construcción, no ve).
...
Además, SAC emplea en su formulación estándar dos redes crítico
(\textit{clipped double-Q}) y utiliza el mínimo de sus estimaciones para
mitigar la sobrestimación del valor Q, técnica heredada de TD3
\citep{Fujimoto2018}. Se adopta esta configuración canónica
($n_{\text{critics}}=2$), sin tratarla como un hiperparámetro a ajustar.
...
La elección de 24 horas como horizonte no es arbitraria: es la escala natural
del problema. Tanto el precio PVPC como la generación fotovoltaica y el
consumo residencial tienen una periodicidad dominante diaria, de modo que una
ventana de 24 horas contiene un ciclo completo de arbitraje (el valle y el
pico de precio, y el día y la noche solares). Coincide además con la ventana
de información del mercado español (REE publica el PVPC del día siguiente a
las 20:30\,h, y el despacho diario de OMIE opera precisamente a esa escala),
por lo que es el horizonte estándar de la planificación horaria (Nivel~3) en
la gestión de almacenamiento. Alargarlo aportaría poco: más allá del día
siguiente no hay nueva estructura de precios que explotar y el error de
pronóstico crece, mientras que el efecto de las decisiones sobre jornadas
posteriores lo captura el agente a través del factor de descuento
($\gamma=0{,}99$, Sección~\ref{sec:fundamentos}).

La principal ventaja del MPC es que puede incorporar explícitamente las
restricciones físicas del sistema (límites de \acrshort{soc}, potencia máxima) y optimizar
...
\begin{table}[H]
\centering
\caption{Stack tecnológico de la plataforma LeaLink.}
\label{tab:stack}
...
Fuente precios & ESIOS \acrshort{api} (REE) & Indicadores 1001 y 1739 \\
...
por sus
implementaciones auditadas con verificación estática de tipos, soporte nativo
para \texttt{VecNormalize} y acceso programático al \textit{replay buffer},
que es lo que permite implementar el protocolo de inicialización con
demostraciones del MPC (DAWN) sobre los algoritmos \textit{off-policy}. HiGHS garantiza optimalidad global
...
se obtiene del indicador 1006 de ESIOS~\citep{ESIOS}, que publica coeficientes
...
$15 \times 3500$\,kWh/año $=$ 52.500\,kWh/año.
...
multiplica por 3500\,kWh/año y por las quince viviendas, y sobre la
...
\begin{equation}
  ...
  \label{eq:ar1}
\end{equation}
Los parámetros del ruido se fijan por argumentos de plausibilidad física, no
por calibración contra medidas (que no existen para la comunidad modelada, al
ser el dataset un agregado sintético; Sección~\ref{sec:limitaciones}): la
magnitud $\sigma=0{,}05$ refleja que la agregación de quince viviendas suaviza
la variabilidad del perfil individual pero no la elimina; la persistencia
$\rho=0{,}50$ evita el ruido blanco hora a hora, físicamente irreal, porque
los desvíos de consumo de una comunidad (una ola de frío, una ausencia)
persisten durante varias horas; y el recorte a $[0{,}7,\,1{,}3]$ acota los
desvíos extremos al rango verosímil de una demanda residencial agregada.
...
ruido propio ($\sigma=0{,}03$, menor que el del consumo porque la producción
FV de instalaciones colindantes está fuertemente correlacionada y sus desvíos
locales, como el sombreado puntual, son de menor amplitud relativa). Por último convierte los precios de compra
...
del ruido, garantizando la trazabilidad. La Figura~\ref{fig:pipeline_etl}
resume el recorrido completo del dato, desde las fuentes oficiales hasta la
entrada normalizada de la red del agente.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{img/pipeline_etl.png}
  \caption{Pipeline de datos: fuentes crudas oficiales (ESIOS 1006/1001/1739 y
  PVGIS), proceso ETL y escalado comunitario, construcción de la observación en
  el entorno de simulación y normalización VecNormalize previa a la red del
  agente.}
...
generación y consumo a nivel anual. Con un consumo medio de 3500\,kWh/año
por vivienda (coherente con la media eléctrica del hogar español según los
estudios SPAHOUSEC del IDAE \citep{IDAE_SPAHOUSEC}) y quince
viviendas, el consumo agregado asciende a 52.500\,kWh/año. Aplicando las
1250 horas equivalentes de irradiancia anual en Galicia, la potencia necesaria es:
\begin{equation}
  P_{\text{solar}} = \frac{\text{52.500 kWh/año}}{\text{1250 h/año}} = \SI{42}{kWp}
...
energética de 2022 (precios PVPC horarios de hasta \num{0,95}\,€/kWh) y el periodo
...
La solución adoptada es el muestreo aleatorio por semanas completas. Las 134
semanas de 168 horas del dataset se distribuyen aleatoriamente sin reemplazo
(semilla fija 42) en 50 semanas de evaluación y 84 de entrenamiento, de modo
que ambos pools quedan repartidos a lo largo de todo el rango temporal y las
dos distribuciones resultan homogéneas. El test
...
La Figura~\ref{fig:topologia} muestra la topología física que el simulador
reproduce: los cuatro flujos de la batería que decide el agente (cs, cm, dc,
dr) y los flujos automáticos del balance (autoconsumo directo, compra de
déficit y vertido de excedente), que no pasan por la batería.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{img/topologia_4flujos.png}
  \caption{Topología física de la comunidad y de los cuatro flujos de la
  batería. En cada hora la batería o carga (cs, cm) o descarga (dc, dr); los
  flujos grises son consecuencia automática del balance horario y no pasan por
  la batería.}
  \label{fig:topologia}
\end{figure}
...
Los valores de estos parámetros proceden de las hojas de características de
sistemas BESS comerciales de química LFP, igual que la vida útil empleada en
la calibración de la degradación (Sección~\ref{sub:calibracion-degradacion}):
la eficiencia \textit{round-trip} resultante (90,25\,\%, al aplicar
$\eta=0{,}95$ en cada sentido) se sitúa en el rango típico del 88--95\,\% de
un sistema estacionario completo, incluyendo las pérdidas de conversión del
inversor; y la autodescarga de $0{,}004\,\%/\text{h}$ (en torno al 3\,\% al
mes) corresponde al orden de magnitud característico de la química LFP, de
autodescarga baja frente a otras químicas de litio.
...
degradación electroquímica \citep{Vetter2005, Xu2016}. El coeficiente
$C_{\text{base}} = 0{,}005$\,€/kWh no debe confundirse con el coste efectivo
de degradación: una vez modulado por los factores de estrés, el coste efectivo
medio resulta de unos $0{,}006$\,€/kWh, y es ese valor el que se calibra
contra la amortización realista de la batería en la
Sección~\ref{sub:calibracion-degradacion}.
...
$N_{\text{ciclos}}\!\approx\!8\,000$ ciclos completos hasta el 80\,\% de SoH,
valor típico de las hojas de características de fabricantes de celdas LFP de
aplicación estacionaria (batería fija de edificio o instalación, cuyo ciclado
es más suave que el de un vehículo eléctrico), la
ecuación~\eqref{eq:calibracion}
...
\textbf{Resultado:} $+51{,}45$\,€/semana (IC95 $[44{,}07;\,60{,}29]$) bajo el
protocolo unificado de evaluación (50 semanas \textit{held-out}, SoC inicial
50\,\%, intervalo de confianza por \textit{bootstrap};
Sección~\ref{sub:seleccion}). El intervalo es ancho porque refleja la
variabilidad estacional entre las semanas del año, no incertidumbre del
controlador (con previsión perfecta el resultado de cada semana es
determinista).
...
La estructura y la magnitud de este ruido se justifican por el comportamiento
característico del error de pronóstico a día siguiente: es mínimo en la
primera hora y crece con el horizonte, de ahí el $\sigma$ creciente del 5 al
25\,\% en la generación FV, cuyo error (de origen meteorológico) es además el
más persistente ($\rho=0{,}7$: un frente nuboso mal previsto afecta a horas
consecutivas). El consumo agregado, en cambio, recibe un error de magnitud
constante ($\sigma=15\,\%$) y menos persistente ($\rho=0{,}3$), coherente con
una demanda residencial cuyos desvíos son más erráticos y de menor memoria que
los meteorológicos. Los valores concretos son supuestos del escenario, no
mediciones; su elección no es crítica para las conclusiones, porque la
ablación F6 (Sección~\ref{sub:resultados}) barre el ruido de consumo entre el
5 y el 25\,\% y el resultado cualitativo del trabajo se mantiene en todo el
rango.

\textbf{Resultado:} $+43{,}39$\,€/semana (IC95 $[41{,}24;\,45{,}57]$, media
sobre diez semillas de ruido).

El coste agregado de la incertidumbre del pronóstico es
$51{,}45 - 43{,}39 = 8{,}06$\,€/semana (15{,}7\,\%), englobando las tres
...
\paragraph{Criterio de selección de los candidatos.} Los candidatos de la
exploración no se eligen por preferencia, sino como representantes canónicos de
las familias algorítmicas de la Sección~\ref{sec:fundamentos}: \acrshort{dqn}
representa a los métodos basados en valor, por ser el algoritmo de referencia
del dominio desde \citet{Mnih2015} y el más citado en las revisiones de RL
aplicado a sistemas energéticos \citep{Perera2021, Vazquez2019};
\acrshort{ppo} representa a los \textit{policy gradient on-policy}, por ser el
estándar de facto de esa familia por su robustez de entrenamiento
\citep{Schulman2017}, y se entrena sobre el mismo espacio discreto para aislar
el efecto del mecanismo de aprendizaje del efecto del espacio de acciones. La
tercera familia (\textit{actor-critic off-policy} continuo, cuyo representante
es \acrshort{sac}) no se explora como agente autónomo, sino que se reserva
para la arquitectura residual, decisión que se justifica en la
Sección~\ref{sub:justificacion}; la suite de validación completa después esa
familia con sus otros dos miembros estándar (TD3 y DDPG) y con el propio SAC
puro como ablación (Sección~\ref{sub:suite}).
...
La elección de \acrshort{sac} como algoritmo de esa arquitectura residual se
deriva de tres requisitos que la propia corrección impone: (a) espacio de
acción \emph{continuo}, porque la razón de ser del residual son los ajustes
finos de potencia que el espacio discreto no puede representar; (b) naturaleza
\emph{off-policy} con \textit{replay buffer}, imprescindible para inyectar las
demostraciones del MPC (protocolo DAWN, Sección~\ref{sub:dawn}), algo
estructuralmente imposible en un método \textit{on-policy} como PPO; y (c)
exploración regulada dentro de un rango acotado, que la regularización por
entropía de SAC proporciona de forma automática \citep{Haarnoja2018}. De los
tres \textit{actor-critic off-policy} continuos estándar (SAC, TD3, DDPG),
SAC es el único que cumple los tres requisitos por construcción; TD3 y DDPG
se incluyen después en la suite de validación precisamente para confirmar esa
elección por comparación directa (Sección~\ref{sub:suite}).

Cabe preguntarse por qué no se exploró antes SAC como agente autónomo, sin
MPC base. La razón es de asignación de esfuerzo: la evidencia acumulada en la
exploración (DQN y PPO por debajo del MPC realista) y la literatura
\citep{Zhan2023} indicaban que ningún agente de RL puro superaría a un MPC
bien calibrado, por lo que el esfuerzo experimental se dirigió a la
arquitectura híbrida en lugar de a un tercer intento de RL puro. La hipótesis
no se dio por supuesta: la suite de validación incluye SAC puro (versión F5)
exactamente para cerrarla con datos, y su resultado ($26{,}94$\,€/semana, el
peor agente de toda la suite; Sección~\ref{sub:resultados}) confirma a
posteriori que explorar SAC autónomo no habría cambiado la decisión.

De esta exploración nace además el registro de experimentos
(\texttt{config/experimentos.yaml}) que define las versiones de la suite de
validación de la Sección~\ref{sub:suite}: cada familia y cada versión validada
responde a una hipótesis generada en esta fase (el n-step y el $\gamma$ de DQN,
el decaimiento de entropía de PPO, la forma y el rango de la corrección
residual), de modo que la suite no es un barrido ciego de hiperparámetros sino
la confirmación estadística, sobre el dimensionado definitivo, de lo aprendido
...
se degradan a lo largo de la vida útil de la batería (15 a 20 años para una
LFP estacionaria, es decir, fija en la instalación). Un
...
ya que $\text{flow} \times (1 + 0) = \text{flow}$: el punto de partida del
aprendizaje y el modo de respaldo del sistema son exactamente el MPC. Para el
agente ya entrenado la garantía es de \emph{acotación}, no de identidad: la
corrección no puede exceder $\pm\delta_{\max}$ sobre cada flujo, lo que limita
estructuralmente cuánto puede alejarse del MPC (en la suite de validación, la
peor configuración residual observada queda $1{,}07$\,€/semana por debajo del
MPC, un $2{,}5\,\%$, frente a los 4--16\,€/semana que pierden los agentes de
RL puro). Ningún algoritmo de RL puro ofrece ninguna de las dos propiedades.
...
\bottomrule
\multicolumn{6}{@{}p{\dimexpr\textwidth-2\tabcolsep}@{}}{\small *Descartados
  como controlador final tras la fase de exploración; se reentrenan no obstante
  en la suite de validación (Sección~\ref{sub:suite}) como familias
  comparativas, para confirmar con estadística las limitaciones aquí
  diagnosticadas.}
\end{tabularx}
\end{table}

La Figura~\ref{fig:taxonomia} sitúa a todos los controladores del ecosistema
bajo una misma interfaz raíz (\texttt{BaseController}), organizados en tres
familias: clásicos y \textit{baselines}, RL puro y control híbrido residual.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{img/taxonomia_controladores.png}
  \caption{Taxonomía de los controladores implementados. Todas las familias
  comparten el contrato \texttt{solve(state, forecast)}, lo que permite
  evaluarlas bajo el mismo protocolo; el MPC suministra la política base
  u\_mpc(t) a la familia residual.}
  \label{fig:taxonomia}
\end{figure}
...
Nota de nomenclatura: las cifras de esta subsección corresponden a la
\emph{fase de diseño} (dimensionado preliminar de 50\,kWp/100\,kWh, una
semilla) y son indicativas; las iteraciones se citan como v2--v5 siguiendo el
Anexo~\ref{app:historial}. En la \emph{suite de validación}
(Sección~\ref{sub:suite}), los identificadores SAC-A, SAC-B y SAC-C designan
los reentrenamientos confirmatorios de estas tres arquitecturas (aditiva,
multiplicativa amplia y multiplicativa fina) sobre el dimensionado definitivo
de 42\,kWp/80\,kWh, cuyos resultados con tres semillas y diez semillas de
ruido se reportan en la Sección~\ref{sub:resultados}.

\paragraph{Iteración 1 (v2/v3): residual aditivo.}
...
\paragraph{Iteración 2 (v4): residual multiplicativo amplio.}
...
\paragraph{Iteración 3 (v5): residual multiplicativo fino (arquitectura final).}
...
arquitectura que se consolida como controlador propuesto: su reentrenamiento
confirmatorio sobre el dimensionado definitivo (versión SAC-C de la suite de
validación) alcanza $+44{,}05$\,€/semana frente a los $+43{,}39$ del MPC
realista, una mejora de $+0{,}66$\,€/semana estadísticamente significativa
(Sección~\ref{sub:resultados}).
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{img/secuencia_inferencia.png}
  \caption{Diagrama de secuencia de la inferencia del Residual SAC sobre el
  entorno simulado: el MPC resuelve su LP y propone la acción base, el actor
  calcula la corrección $\Delta$ (determinista, con normalización congelada) y
  la fusión multiplicativa se aplica al simulador físico-económico.}
...
conocer la acción base sobre la que propone su corrección; es una extensión
natural del esquema residual de \citet{Johannink2019}, en la que el
condicionamiento a la acción base se hace explícito en la observación.
...
\begin{table}[H]
\centering
\caption{Hiperparámetros del Residual SAC (configuración final SAC-C).}
...
\texttt{delta\_max} & 0{,}15 & $\pm15\,\%$; meseta 0{,}05--0{,}20, degradación en 0{,}30 (barrido F2) \\
...
cuyas tres semillas rinden casi idéntico por la garantía de suelo: métrica
interna de $41{,}08$, $41{,}14$ y $41{,}11$\,€/semana para las semillas 42,
1337 y 2024).
...
de la exploración. La suite se ha completado en su totalidad: 31 versiones
entrenadas y evaluadas bajo el protocolo unificado (tres semillas de
entrenamiento, diez semillas de ruido de evaluación, 50 semanas
\textit{held-out}, intervalos de confianza al 95\,\% por \textit{bootstrap} y
test de Wilcoxon pareado por semana y semilla de ruido frente al MPC realista). Los resultados
agregados se presentan en la Sección~\ref{sub:resultados}; el detalle completo
de cada versión (media, IC95, $p$ e hiperparámetros) se recoge en el
Anexo~\ref{app:suite_completa}, y los entornos de entrenamiento y las
condiciones de reproducibilidad en el Anexo~\ref{app:reproducibilidad}.

\subsection{Resultados Experimentales}
\label{sub:resultados}

El entrenamiento converge con rapidez según los registros de evaluación
periódica (\texttt{evaluations.npz} de cada ejecución): la meseta se alcanza
en torno a los 100.000 pasos y se mantiene estable hasta completar el millón,
comportamiento coherente con el espacio de corrección acotado a $\pm15\,\%$,
porque desde el primer punto de control el agente rinde ya cerca de su nivel
final (la corrección acotada no puede alejarse del suelo del MPC). El mejor
\textit{checkpoint} de la semilla desplegada (1337) se selecciona en el paso
280.000. No se reproduce aquí la curva de entrenamiento: en el protocolo de
este trabajo el objeto evaluado es el modelo final bajo la evaluación
unificada, y la dinámica del entrenamiento (idéntica entre semillas por la
garantía de suelo, Cuadro~\ref{tab:seeds}) no aporta información adicional
sobre el rendimiento desplegado. El Cuadro~\ref{tab:resultados} recoge los resultados finales de los
\textit{baselines} y de la mejor versión de cada familia bajo el protocolo de
evaluación unificado; el detalle de las 31 versiones se encuentra en el
Anexo~\ref{app:suite_completa}.

\begin{table}[H]
\centering
\caption{Resultados de la suite de validación: \textit{baselines} y mejor
versión de cada familia (€/semana, beneficio marginal frente a IDLE; 50 semanas
\textit{held-out} $\times$ 10 semillas de ruido; IC95 por \textit{bootstrap};
$p$ del test de Wilcoxon pareado por semana y semilla de ruido frente al MPC realista).}
\label{tab:resultados}
\small
\begin{tabular}{lrrcc}
\toprule
\textbf{Controlador} & \textbf{€/sem} & \textbf{$\Delta$ MPC} &
\textbf{IC95} & \textbf{$p$} \\
\midrule
MPC oráculo (cota superior) & $51{,}45$ & $+8{,}06$ & $[44{,}07;\,60{,}29]$ & n/a \\
TD3 residual (TD3-4) & $44{,}17$ & $+0{,}79$ & $[42{,}02;\,46{,}37]$ & $1{,}5\cdot10^{-37}$ \\
\textbf{Residual SAC (SAC-C)} & $\mathbf{44{,}05}$ & $\mathbf{+0{,}66}$ &
$\mathbf{[41{,}90;\,46{,}25]}$ & $\mathbf{1{,}5\cdot10^{-23}}$ \\
DDPG residual (mejor: DDPG-1) & $43{,}44$ & $+0{,}05$ & $[41{,}32;\,45{,}60]$ & $0{,}95$ \\
MPC realista (referencia) & $43{,}39$ & --- & $[41{,}24;\,45{,}57]$ & --- \\
DQN (mejor: DQN-1, $n{=}1$) & $39{,}36$ & $-4{,}02$ & $[37{,}21;\,41{,}62]$ & $5{,}0\cdot10^{-67}$ \\
PPO discreto (mejor: PPO-D4) & $39{,}35$ & $-4{,}04$ & $[37{,}09;\,41{,}70]$ & $2{,}4\cdot10^{-60}$ \\
PPO continuo (mejor: PPO-C3) & $38{,}31$ & $-5{,}07$ & $[36{,}08;\,40{,}66]$ & $6{,}5\cdot10^{-45}$ \\
Heurístico & $30{,}49$ & $-12{,}90$ & $[28{,}14;\,32{,}91]$ & $4{,}6\cdot10^{-75}$ \\
SAC puro (F5) & $26{,}94$ & $-16{,}44$ & $[25{,}53;\,28{,}40]$ & $1{,}3\cdot10^{-83}$ \\
IDLE (sin batería) & $0{,}00$ & $-43{,}39$ & --- & --- \\
\bottomrule
\multicolumn{5}{@{}p{\dimexpr\textwidth-2\tabcolsep}@{}}{\small Dimensionado
  definitivo 42\,kWp\,/\,80\,kWh en todas las filas. Cada agente se entrena con
  tres semillas y se despliega la mejor (Sección~\ref{sub:seleccion}); las
  ablaciones F2/F4/F6 del Anexo~\ref{app:suite_completa} usan una semilla.}
\end{tabular}
\end{table}

\subsubsection{Resultado central}

El resultado central del trabajo es que el \textbf{Residual SAC (SAC-C,
$+44{,}05$\,€/semana) supera al MPC realista ($+43{,}39$\,€/semana)} en
$+0{,}66$\,€/semana ($+1{,}5\,\%$), con IC95 $[41{,}90;\,46{,}25]$ y
significancia estadística muy alta en el contraste pareado por semana y
semilla de ruido (Wilcoxon, $p=1{,}45\cdot10^{-23}$): la mejora es pequeña en magnitud pero
extraordinariamente consistente, porque se repite semana a semana en las 500
evaluaciones realizadas (50 semanas $\times$ 10 semillas de ruido). Conviene
leer el margen con honestidad: el MPC realista es un controlador muy fuerte, y
el techo alcanzable es la brecha de pronóstico de $8{,}06$\,€/semana, de la que
el residual recupera en torno al 8\,\%. El valor de la arquitectura no reside
solo en esa mejora media, sino en su combinación con la garantía de suelo
(Sección~\ref{sub:justificacion}) y con una aportación creciente cuando el
pronóstico se degrada (ablación F6, más abajo). Ningún controlador de RL puro
(DQN, PPO en sus dos variantes, SAC directo) consigue siquiera igualar al MPC.

\subsubsection{Consistencia entre semillas de entrenamiento}

\begin{table}[H]
\centering
\caption{Consistencia inter-entrenamiento del Residual SAC (SAC-C): métrica
interna de entrenamiento por semilla (€/semana).}
\label{tab:seeds}
\begin{tabular}{lc}
\toprule
\textbf{Semilla de entrenamiento} & \textbf{Métrica interna} \\
\midrule
42   & $41{,}08$ \\
...
Las tres semillas de entrenamiento difieren entre sí en solo
$0{,}06$\,€/semana (un $0{,}15\,\%$), evidencia empírica de que la garantía de
suelo elimina en la práctica la varianza inter-semilla característica del RL
profundo: partir del MPC y corregirlo de forma acotada deja poco espacio para
que la aleatoriedad del entrenamiento produzca modelos dispares. Se despliega
la semilla 1337 (mejor métrica interna) y su rendimiento publicado
($+44{,}05$) es la media agregada sobre las diez semillas de ruido de
evaluación; la significancia del resultado queda establecida por el contraste
pareado sobre los 500 pares semana-ruido (Wilcoxon, $p<0{,}001$).

\subsubsection{Estudios del ganador}

\paragraph{Forma del residual (SAC-A/B/C).} Los reentrenamientos
confirmatorios de las tres arquitecturas de la fase de diseño reproducen su
ordenación sobre el dimensionado definitivo: la aditiva SAC-A queda en
$42{,}47$\,€/semana ($-0{,}91$ frente al MPC, $p=1{,}8\cdot10^{-28}$), la
multiplicativa amplia SAC-B en $42{,}31$ ($-1{,}07$) y solo la multiplicativa
fina SAC-C supera al MPC ($+0{,}66$). El dato confirma el diagnóstico de la
fase de diseño: la forma multiplicativa con rango fino no es una opción entre
varias equivalentes, sino la única de las tres que aporta valor positivo; las
otras dos son significativamente peores que el propio MPC que corrigen.

\paragraph{Barrido del rango de corrección (ablación F2).} El
Cuadro~\ref{tab:f2} muestra el efecto de $\delta_{\max}$ manteniendo el resto
de la configuración de SAC-C.

\begin{table}[H]
\centering
\caption{Barrido del rango de corrección $\delta_{\max}$ (ablación F2).}
\label{tab:f2}
\begin{tabular}{lccccc}
\toprule
$\delta_{\max}$ & $0{,}05$ & $0{,}10$ & $0{,}15$ (SAC-C) & $0{,}20$ & $0{,}30$ \\
\midrule
€/semana & $43{,}79$ & $44{,}01$ & $\mathbf{44{,}05}$ & $43{,}78$ & $42{,}43$ \\
$\Delta$ vs.\ MPC & $+0{,}41$ & $+0{,}63$ & $\mathbf{+0{,}66}$ & $+0{,}40$ & $-0{,}95$ \\
\bottomrule
\multicolumn{6}{@{}l@{}}{\small F2-d05/d10/d20/d30: una semilla de
entrenamiento; SAC-C: tres semillas.}
\end{tabular}
\end{table}

La ventaja sobre el MPC forma una meseta robusta en $\delta_{\max}\in[0{,}05;
0{,}20]$ (todas las versiones lo superan) y se degrada con claridad en
$0{,}30$, donde el agente puede desviarse demasiado de la política base. El
resultado del trabajo no depende, por tanto, de un ajuste fino afortunado del
rango de corrección.

\paragraph{Arranque guiado DAWN (ablación F4).} La ablación F4 elimina el
\textit{warmup} DAWN manteniendo el resto de la configuración de SAC-C: el
resultado final es $+44{,}04$\,€/semana ($+0{,}65$ sobre el MPC),
estadísticamente indistinguible del $+44{,}05$ de SAC-C. El dato obliga a
matizar la tesis inicial: \textbf{DAWN no mejora el rendimiento final} del
residual multiplicativo, porque la propia arquitectura (corrección acotada
sobre una política ya optimizada) evita por sí sola la patología de arranque
en frío: los registros de evaluación periódica del entrenamiento
(\texttt{evaluations.npz} de cada ejecución) muestran que, incluso sin
\textit{warmup}, el agente rinde desde el primer punto de control (paso
20.000) cerca de su nivel final ($39{,}1$ frente a $39{,}9$ con DAWN, sobre
una meseta final de $\approx41$ en la métrica interna). El valor de DAWN queda así acotado a lo
operativo: arrancar con el \textit{replay buffer} poblado por la política del
MPC y las estadísticas de normalización calibradas, algo relevante en un
escenario de reentrenamiento en producción, donde el sistema debe seguir
operando mientras aprende, pero no en la cifra final de laboratorio.

\paragraph{Sensibilidad al error de pronóstico (ablación F6).} La ablación F6
reentrena y evalúa la configuración de SAC-C bajo dos niveles alternativos de
ruido de consumo; cada fila del Cuadro~\ref{tab:f6} compara al agente con el
MPC realista \emph{que ve el mismo nivel de ruido}, de modo que la comparación
es justa en los tres escenarios.

\begin{table}[H]
\centering
\caption{Sensibilidad al error de pronóstico de consumo (ablación F6). La
fila central corresponde a la configuración estándar (F6-s15 es exactamente
SAC-C).}
\label{tab:f6}
\begin{tabular}{lcccc}
\toprule
$\sigma_{\text{consumo}}$ & \textbf{MPC realista} & \textbf{Residual SAC} &
\textbf{$\Delta$} & \textbf{$p$} \\
\midrule
5\,\%  & $48{,}27$ & $47{,}48$ & $-0{,}79$ & $1{,}9\cdot10^{-68}$ \\
15\,\% & $43{,}39$ & $44{,}05$ & $+0{,}66$ & $1{,}5\cdot10^{-23}$ \\
25\,\% & $38{,}59$ & $40{,}02$ & $+1{,}42$ & $8{,}6\cdot10^{-61}$ \\
\bottomrule
\end{tabular}
\end{table}

La aportación del residual crece de forma monótona con el error de pronóstico
($-0{,}79 \rightarrow +0{,}66 \rightarrow +1{,}42$). Con un pronóstico casi
perfecto el MPC es ya casi óptimo y el residual no encuentra error sistemático
que corregir (de hecho, estorba); cuanto peor es el pronóstico, mayor es el
margen sistemático aprendible. Este resultado confirma el mecanismo del
Argumento~1 de la Sección~\ref{sub:justificacion} y delimita el dominio de
utilidad del sistema: comunidades con pronósticos imperfectos, que es
precisamente el caso realista.

\subsubsection{Familias comparativas}

\paragraph{DQN (DQN-1\,\ldots\,6).} La mejor versión validada es DQN-1
($n$-step${}=1$) con $39{,}36$\,€/semana ($-4{,}02$ frente al MPC,
$p=5{,}0\cdot10^{-67}$), seguida de cerca por DQN-6 ($n{=}4$, $39{,}27$).
Estas versiones son reentrenamientos del registro de experimentos sobre el
dimensionado definitivo, no las versiones DQN-A/B/C de la fase de exploración.
La validación obliga a matizar una hipótesis de aquella fase: el
$n$-step${}\approx 8$ que allí pareció óptimo no se reproduce (con $n{=}8$ se
obtienen $37{,}24$ y $38{,}34$), y el horizonte largo $n{=}24$ es claramente el
peor ($31{,}65$). La conclusión robusta no es el valor concreto de $n$-step,
que resulta sensible al resto de la configuración, sino la estructural: toda
la familia queda entre 4 y 12\,€/semana por debajo del MPC, con el error de
cuantización del espacio de acciones discreto como causa de fondo.

\paragraph{PPO discreto (PPO-D1\,\ldots\,5).} La validación confirma la
hipótesis de la exploración sobre el colapso de entropía: las dos versiones
con decaimiento conjunto de entropía y tasa de aprendizaje son las mejores de
la familia (PPO-D4, $39{,}35$\,€/semana, y PPO-D3, $38{,}91$), mientras que
las tres de entropía constante quedan por detrás ($37{,}14$ a $37{,}96$), con
la de mayor entropía fija (PPO-D5, ent $0{,}01$) como la peor. Aun así, la
mejor versión queda $4{,}04$\,€/semana por debajo del MPC
($p=2{,}4\cdot10^{-60}$), prácticamente empatada con el mejor DQN ($39{,}36$):
las dos familias discretas topan en el mismo techo, coherente con un error de
cuantización estructural que ningún ajuste de hiperparámetros elimina.

\paragraph{PPO continuo (PPO-C1\,\ldots\,3).} Las tres versiones quedan en
$38{,}14$, $33{,}66$ y $38{,}31$\,€/semana: pasar a acción continua elimina el
error de cuantización pero no basta para alcanzar al MPC (unos 5\,€/semana por
debajo). Al ser \textit{on-policy}, PPO no puede aprovechar demostraciones del
MPC, lo que aísla la conclusión: el mérito del controlador final no está en el
espacio de acción continuo, sino en la arquitectura residual.

\paragraph{SAC puro (F5).} El mismo algoritmo del controlador ganador,
entrenado directamente sobre el entorno continuo sin MPC base ni DAWN, obtiene
$26{,}94$\,€/semana ($-16{,}44$, $p=1{,}3\cdot10^{-83}$), el peor agente de
toda la suite e inferior incluso al heurístico. Es la evidencia más nítida de
que el mérito del resultado central corresponde a la \emph{arquitectura
residual} y no al algoritmo SAC en sí.

\paragraph{TD3 residual (TD3-1\,\ldots\,4).} La familia TD3 confirma que la
arquitectura residual funciona con otros algoritmos \textit{off-policy}:
TD3-1/2/3 ($\delta_{\max}=0{,}15$) resultan estadísticamente indistinguibles
del MPC realista ($p=0{,}81$, $0{,}12$ y $0{,}07$), y TD3-4
($\delta_{\max}=0{,}10$) alcanza $+44{,}17$\,€/semana ($+0{,}79$,
$p=1{,}5\cdot10^{-37}$), la mejor media puntual de toda la suite, por encima
incluso de SAC-C ($+44{,}05$). Se mantiene no obstante SAC-C como controlador
desplegado, por tres razones: (a) la diferencia entre ambos no es
estadísticamente significativa (IC95 $[42{,}02;\,46{,}37]$ frente a
$[41{,}90;\,46{,}25]$, ampliamente solapados); (b) la familia SAC es más
robusta al hiperparámetro, con cinco de sus diez versiones superando al MPC
con significancia frente a una de cuatro en TD3, cuyo resultado depende
críticamente del valor de $\delta_{\max}$; y (c) es la familia caracterizada
en profundidad (todas las ablaciones F2--F6) y la ya integrada en la
plataforma (exportación ONNX, Sección~\ref{sec:onnx}). TD3-4 queda documentado
como alternativa de despliegue equivalente.

\paragraph{DDPG residual (DDPG-1/2).} Las dos versiones quedan
estadísticamente empatadas con el MPC realista: DDPG-1 (sin ruido de
exploración explícito) obtiene $43{,}44$\,€/semana ($+0{,}05$, $p=0{,}95$) y
DDPG-2 (ruido Ornstein-Uhlenbeck) $43{,}36$ ($-0{,}03$, $p=0{,}20$). Sin doble
crítico ni regularización por entropía, DDPG no extrae valor positivo del
margen de corrección, pero tampoco cae por debajo del MPC: la garantía de
suelo de la arquitectura residual sostiene incluso al algoritmo más frágil de
los tres \textit{off-policy} probados. El patrón agregado de la comparativa
es nítido: la arquitectura residual mantiene a todos los algoritmos en el
nivel del MPC o por encima, y solo los mejor regularizados (SAC, y TD3 con el
rango adecuado) convierten ese suelo en mejora significativa.

\subsubsection{Síntesis: hipótesis confirmadas y matizadas}

La suite confirma las hipótesis centrales del trabajo: (i) ningún agente de RL
puro (valor, \textit{policy-gradient} discreto o continuo, SAC directo) iguala
al MPC bien calibrado; (ii) el residual multiplicativo fino lo supera con
significancia estadística; (iii) el mérito es de la arquitectura residual y no
del algoritmo (F5); y (iv) el resultado es robusto al rango de corrección
(meseta de F2). Dos hipótesis intermedias quedan en cambio matizadas por los
datos, y se reportan como tales: el $n$-step óptimo de DQN no es estable entre
configuraciones, y DAWN no mejora el rendimiento final (su valor es operativo,
no de resultado). Por último, la ablación F6 delimita el dominio de utilidad:
la ventaja del residual existe cuando el pronóstico es imperfecto y crece con
su error, el escenario realista de una comunidad energética rural.
...
(\texttt{VecNormalize}) se exportan a un fichero \texttt{.npz} acompañante
(\texttt{vec\_normalize\_*.npz}), ya que la
...
inferencia con una latencia del orden de milisegundos por paso (despreciable
frente al paso de control horario), sin necesidad de PyTorch ni Gymnasium; la
equivalencia numérica entre el actor PyTorch y el exportado a ONNX se verifica
con los tests \texttt{test\_onnx\_equivalence}. La acción final se reconstruye aplicando la corrección residual
...
que ejecuta el MPC y aplica la corrección del modelo en tiempo real. La
Figura~\ref{fig:arquitectura_prod} completa la vista arquitectónica de la
Figura~\ref{fig:arquitectura} con las dos capas de producción: el despliegue
\textit{edge} y la plataforma SaaS que consume sus decisiones.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{img/arquitectura_produccion.png}
  \caption{Arquitectura de la plataforma, parte 2: producción \textit{edge}
  (exportación ONNX, inferencia ligera y bucle horario) y capa SaaS (recepción
  de decisiones, histórico y cierres mensuales).}
...
garantizar la reproducibilidad del entorno. El acceso se organiza en dos roles
globales (Superadmin y Usuario), cuyas funciones concretas se detallan al
...
\textbf{Modelo de datos.} El núcleo del esquema se compone de siete entidades
de dominio (ocho clases, al modelarse \texttt{AccesoVivienda} como entidad
asociativa), recogidas en la Figura~\ref{fig:modelo_datos}; a ellas se suma
\texttt{OperacionHoraria}, el registro horario de las decisiones del
controlador \textit{edge} (Sección~\ref{sub:api_edge_saas}), con lo que el
modelo completo consta de nueve clases. \texttt{Comunidad} agrupa sus \texttt{Vivienda}s
...
Conviene explicitar el alcance del prototipo en este punto: el dispositivo de
borde no está conectado a APIs de datos en tiempo real, sino que se emula con
un proceso que recorre el dataset histórico (los mismos ficheros de ESIOS y
PVGIS con los que se entrenó y evaluó el controlador) y alimenta la plataforma
a través de exactamente los mismos endpoints que usaría el hardware real. Los
conectores a las fuentes en vivo están diseñados con la misma interfaz que el
dataset histórico, pero su activación (que requiere únicamente configurar los
tokens de acceso) queda como trabajo futuro (Capítulo~\ref{chap:futuro}).
...
que la cooperativa asigna a cada socio. En la implementación actual este
reparto se efectúa de forma multiplicativa sobre el total horario mediante dos
coeficientes energéticos: el \textbf{coeficiente de reparto interno} pondera
el consumo imputado a cada vivienda, y su \textbf{potencia fotovoltaica
instalada} pondera la generación, de modo que la suma de las cuotas reproduce
exactamente el total de la factura común. La evolución prevista de la
plataforma sustituye este reparto energético por \textbf{coeficientes de
reducción de gasto}: la energía de toda la comunidad se netea horariamente
como un único balance (coherente con el neteo del CUPS único sobre el que
decide el controlador) y lo que se reparte entre los vecinos no es energía
sino el \emph{ahorro económico} resultante, ponderado por el número de paneles
solares aportado por cada vivienda. Esta separación entre agregado (que ve el
modelo) y desglose (que ve el usuario) garantiza, en ambos esquemas, que la
capa de presentación nunca altera los datos sobre los que se entrenó y evaluó
el controlador.
...
La Figura~\ref{fig:captura_saas} muestra el panel de control de la plataforma
en funcionamiento para una vivienda concreta: los indicadores de cabecera
(ahorro acumulado, media mensual, porcentaje de ahorro sobre la factura y
cierres registrados), la evolución mensual del ahorro frente a los dos
escenarios de referencia (sin paneles y sin comunidad), la comparativa de
gasto entre los tres escenarios, el origen de la energía consumida en el
último mes (autoconsumo directo, batería comunitaria y red) y la posición de
la vivienda en el ahorro medio mensual de la comunidad.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{img/captura_saas.png}
  \caption{Captura de pantalla del panel de control de la plataforma SaaS
  (LeaLink) para una vivienda: indicadores de ahorro, comparativa de gasto
  entre escenarios (sin paneles, sin comunidad y con comunidad), origen de la
  energía consumida y comparación con el resto de viviendas de la comunidad.}
  \label{fig:captura_saas}
\end{figure}
...
\midrule
0 & Concepción y viabilidad & Sep 2025 & Oct 2025 \\
1 & Setup, arquitectura, YAML & Oct 2025 & Nov 2025 \\
2 & Ingeniería de datos (ETL) & Nov 2025 & Dic 2025 \\
3 & Simulador físico-económico & Dic 2025 & Ene 2026 \\
4 & Controlador MPC & Ene 2026 & Feb 2026 \\
5 & Experimentación DQN y PPO & Feb 2026 & Mar 2026 \\
6 & Residual SAC y tuning & Mar 2026 & May 2026 \\
7 & Evaluación unificada y API & May 2026 & Jun 2026 \\
8 & Documentación y defensa & Jun 2026 & Jul 2026 \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{figure}[H]
  \centering
  % Eje temporal en quincenas: 22 casillas de sep 2025 a jul 2026.
  % Por fase, dos barras: gris (planificado) y azul (real).
  \resizebox{\textwidth}{!}{%
  \begin{ganttchart}[
      x unit=0.62cm,
      y unit title=0.65cm,
      y unit chart=0.62cm,
      canvas/.append style={draw=gray!60},
      title/.append style={draw=gray!60, fill=gray!10},
      title label font=\small\bfseries,
      vgrid={*1{draw=none}, *1{gray!25}},
      hgrid style/.style={gray!25},
      hgrid,
      bar height=0.32,
    ...
    \gantttitle{2025}{8} \gantttitle{2026}{14} \\
    \gantttitle{Sep}{2}\gantttitle{Oct}{2}\gantttitle{Nov}{2}%
    \gantttitle{Dic}{2}\gantttitle{Ene}{2}\gantttitle{Feb}{2}%
    \gantttitle{Mar}{2}\gantttitle{Abr}{2}\gantttitle{May}{2}%
    \gantttitle{Jun}{2}\gantttitle{Jul}{2} \\
    \ganttbar[bar/.append style={fill=ganttplan}, bar top shift=0.12]{F0 Concepción y viabilidad}{1}{3}
    \ganttbar[bar/.append style={fill=ganttreal}, bar top shift=0.56]{}{1}{3} \\
    \ganttbar[bar/.append style={fill=ganttplan}, bar top shift=0.12]{F1 Setup, arquitectura, YAML}{3}{6}
    \ganttbar[bar/.append style={fill=ganttreal}, bar top shift=0.56]{}{3}{6} \\
    \ganttbar[bar/.append style={fill=ganttplan}, bar top shift=0.12]{F2 Ingeniería de datos (ETL)}{5}{8}
    \ganttbar[bar/.append style={fill=ganttreal}, bar top shift=0.56]{}{5}{9} \\
    \ganttbar[bar/.append style={fill=ganttplan}, bar top shift=0.12]{F3 Simulador físico-económico}{7}{10}
    \ganttbar[bar/.append style={fill=ganttreal}, bar top shift=0.56]{}{8}{11} \\
    \ganttbar[bar/.append style={fill=ganttplan}, bar top shift=0.12]{F4 Controlador MPC}{9}{12}
    \ganttbar[bar/.append style={fill=ganttreal}, bar top shift=0.56]{}{10}{13} \\
    \ganttbar[bar/.append style={fill=ganttplan}, bar top shift=0.12]{F5 Experimentación DQN y PPO}{11}{14}
    \ganttbar[bar/.append style={fill=ganttreal}, bar top shift=0.56]{}{12}{16} \\
    \ganttbar[bar/.append style={fill=ganttplan}, bar top shift=0.12]{F6 Residual SAC y tuning}{13}{18}
    \ganttbar[bar/.append style={fill=ganttreal}, bar top shift=0.56]{}{14}{20} \\
    \ganttbar[bar/.append style={fill=ganttplan}, bar top shift=0.12]{F7 Evaluación unificada y API}{17}{20}
    \ganttbar[bar/.append style={fill=ganttreal}, bar top shift=0.56]{}{19}{21} \\
    \ganttbar[bar/.append style={fill=ganttplan}, bar top shift=0.12]{F8 Documentación y defensa}{19}{22}
    \ganttbar[bar/.append style={fill=ganttreal}, bar top shift=0.56]{}{20}{22} \\
  \end{ganttchart}}
  \caption{Diagrama de Gantt del proyecto, con resolución quincenal: para cada
  fase, la barra superior (gris) es la planificación inicial y la inferior
  (azul) la ejecución real. Las mayores desviaciones se concentran en las
  fases 5 y 6 (en torno a un mes entre ambas, por las causas de la
  Sección~\ref{sec:desviaciones}) y se absorben comprimiendo las fases 7 y
  8, manteniendo la entrega en julio de 2026.}
  \label{fig:gantt}
\end{figure}
...
públicas (ESIOS, PVGIS), con dashboards diferenciados por rol que
...
coherente con la literatura reciente \citep{Zhan2023}. La suite de validación
lo confirma para las cuatro variantes puras: DQN sufre un error de
cuantización estructural irresolvible con el espacio discreto de 9 acciones;
PPO, discreto o continuo, no puede aprovechar el protocolo de inicialización
con demostraciones por su naturaleza \textit{on-policy}; y el SAC directo sin
MPC base es el peor agente de toda la suite ($+26{,}94$\,€/semana).

La arquitectura Residual SAC resuelve ambas limitaciones: opera en espacio
continuo y corrige de forma acotada una política base ya optimizada. La
propiedad de garantía de suelo es la contribución técnica más valiosa de la
arquitectura, y conviene enunciarla con precisión: la identidad $\delta=0
\Rightarrow$ MPC es exacta y está verificada por test; para el agente
entrenado, la corrección acotada limita la desviación posible respecto al MPC.
La suite lo respalda empíricamente en sus dos vertientes: las tres semillas de
entrenamiento de SAC-C difieren entre sí en solo $0{,}06$\,€/semana, y de las
16 configuraciones residuales evaluadas (SAC, TD3 y DDPG), las bien
configuradas igualan o superan al MPC y la peor (SAC-B, con el rango de
corrección deliberadamente amplio del barrido) queda solo $1{,}07$\,€/semana
por debajo ($2{,}5\,\%$), frente a los 4 a 16\,€/semana que pierden los
agentes de RL puro.

El MPC realista alcanza $+43{,}39$\,€/semana respecto a IDLE, con un margen
teórico de mejora de $8{,}06$\,€/semana (15{,}7\,\%) hasta el MPC oráculo. El
Residual SAC recupera $+0{,}66$\,€/semana de ese margen, en torno al 8\,\%,
una mejora modesta en magnitud pero estadísticamente robusta ($p<0{,}001$ en
contraste pareado sobre 500 pares semana-ruido) y creciente cuando el
pronóstico se degrada (ablación F6). La cuantificación explícita del margen, y
de qué fracción es capturable, permite contextualizar los resultados sin
sobreestimar el potencial del RL en este dominio.

La suite deja además dos lecciones de honestidad experimental que este trabajo
reporta como tales: el \textit{warmup} DAWN no mejora el rendimiento final del
residual (su valor es operativo, no de resultado), y el horizonte $n$-step que
la exploración señaló como óptimo para DQN no se reproduce en la validación
(Sección~\ref{sub:resultados}). Ambas correcciones ilustran el valor del
esquema exploración-validación adoptado: las hipótesis de la fase barata se
confirman o se corrigen con estadística antes de incorporarse a las
conclusiones.
...
regulada estándar (las Redes de Distribución Cerradas del RD 314/2023 se
reservan a entornos industriales): se sostiene como red interior de un
...
\textbf{Alcance experimental y coste de cómputo.} El barrido de configuraciones y
semillas no se amplió más allá de la suite presentada, por su coste computacional.
El conjunto de entrenamientos y evaluaciones realizado equivale a unas 500 horas de
cómputo en un ordenador de gama media (Anexo~\ref{app:reproducibilidad}), ya que
las familias residuales resuelven un
problema de programación lineal en cada paso de simulación. Ampliar la rejilla (más
versiones, más semillas o ablaciones de componentes internos del algoritmo, como el
número de redes crítico) habría multiplicado ese coste sin aportar información
proporcional a la pregunta de investigación, centrada en el mérito de la corrección
residual sobre el \acrshort{mpc}. Por ello el diseño experimental se dirigió a las
hipótesis relevantes (modo y magnitud de la corrección, coeficiente de exploración y
arranque guiado \textit{DAWN}) en lugar de a una exploración exhaustiva.

\textbf{Heterogeneidad de los entornos de entrenamiento.} Los 31 modelos de la
suite se entrenaron en cuatro entornos de software distintos (combinaciones de
sistema operativo, versión de Python y \textit{build} de PyTorch;
Anexo~\ref{app:reproducibilidad}). La evaluación es determinista e idéntica
para todos ellos (los \textit{baselines} reproducen exactamente los mismos
valores en todas las máquinas), por lo que las cifras publicadas son
directamente comparables; el entrenamiento de RL profundo es, sin embargo,
sensible a la semilla y al entorno de ejecución \citep{Henderson2017}, y esa
varianza se gestiona estadísticamente (tres semillas, IC95 por
\textit{bootstrap} y test de Wilcoxon) en lugar de homogeneizando el hardware.
Las comparaciones internas de la familia residual (barrido $\delta$,
ablaciones DAWN y F6, formas aditiva y multiplicativa) comparten un mismo
entorno de entrenamiento y no se ven afectadas por esta limitación.
...
independientemente de la complejidad del agente. La suite de 76 tests
...
El \textbf{aumento del número de semillas de entrenamiento} (de tres a diez o
más) reforzaría la caracterización de la robustez inter-entrenamiento,
complementando las diez semillas de ruido de evaluación y el contraste de
Wilcoxon pareado que la suite de validación ya incorpora. Dada la mínima
varianza observada entre las tres semillas de SAC-C (rango de
$0{,}06$\,€/semana), el beneficio esperado es de confirmación más que de
descubrimiento.
...
La \textbf{evolución del módulo de reparto} de la plataforma es la extensión
de mayor impacto para el usuario final: el primer paso previsto es sustituir
los coeficientes energéticos actuales por coeficientes de \emph{reducción de
gasto} (reparto del ahorro económico según el número de paneles aportado, con
neteo horario del conjunto de la comunidad, Sección~\ref{sub:panel}); a más
largo plazo, unos \textbf{coeficientes dinámicos} ($\beta_{i,h}$ variables
hora a hora) requerirían modelado individual por vivienda y un módulo de
liquidación más complejo.

La \textbf{previsión con modelos LSTM} para irradiancia y consumo podría
reducir el coste del ruido de pronóstico ($8{,}06$\,€/semana) y elevar el
rendimiento absoluto del sistema. Conviene señalar el matiz que aporta la
ablación F6: un pronóstico mejor beneficia sobre todo al MPC base y reduce la
aportación relativa del residual, de modo que esta línea desplaza valor desde
el componente aprendido hacia el componente de optimización clásica sin restar
rendimiento al conjunto.
...
El Cuadro~\ref{tab:capacidad} recoge la capacidad fotovoltaica instalada en cada
...
\label{app:config}
Se reproduce a continuación el fichero \texttt{config/system.yaml}, fuente
única de configuración del sistema: parámetros físicos de la batería y de la
comunidad, rutas de datos, split de semanas, ruido de ETL y de pronóstico,
MPC, hiperparámetros del Residual SAC y runtime de producción. Se omiten los
comentarios del fichero original; la justificación de cada valor se encuentra
en los capítulos correspondientes de esta memoria.

% Copia sin comentarios generada desde config/system.yaml (solo se eliminan
% líneas de comentario y comentarios de fin de línea; los valores son
% idénticos). Regenerar si cambia la config del repositorio.
\lstinputlisting[language=yaml, basicstyle=\footnotesize\ttfamily]{system_sin_comentarios.yaml}

\chapter{Árbol de Directorios del Repositorio}
\label{app:estructura}
El repositorio se organiza en nueve directorios de primer nivel. El código
fuente vive bajo \texttt{src/}, separado en paquetes por responsabilidad
(núcleo de simulación, entornos, controladores, entrenamiento, evaluación,
producción y plataforma SaaS); la configuración, los datos, los artefactos de
modelo y los resultados experimentales tienen directorios propios, de modo que
el código nunca contiene valores del sistema ni rutas cableadas.

\begin{lstlisting}[language={}, basicstyle=\scriptsize\ttfamily, numbers=none]
TFG/
+-- config/                  Configuracion declarativa (unica fuente)
|   +-- system.yaml            fisica, ETL, MPC y SAC (Apendice B)
|   +-- experimentos.yaml      registry de experimentos y versiones
+-- data/
|   +-- raw/                   4 CSV oficiales crudos (ESIOS y PVGIS)
|   +-- processed/             dataset_final.csv + metadata.json (hashes)
+-- doc/
|   +-- diagramas/             fuentes drawio/PlantUML de las figuras
|   +-- memoria_latex/         esta memoria
+-- docker/                  Imagen edge ARM64 (Dockerfile.edge)
+-- experimentos/            Puntos de entrada: build_dataset.py,
|                              entrenar.sh y eval_rapida.py
+-- models/                  Artefactos de entreno (.zip, .pkl, .onnx, .npz)
|   +-- best/                  campeones por familia de algoritmo
+-- resultados/              resultados.csv canonico + un JSON por version
+-- src/
|   +-- core/                  config, simulator, forecast, observation
|   +-- envs/                  entornos Gymnasium: base, discreto,
|   |                            continuo y residual
|   +-- controllers/           heuristico, MPC, base continua/discreta
|   |                            y controlador Residual SAC
|   +-- buffers/               replay buffer n-step
|   +-- training/              mains por algoritmo, scaffold comun,
|   |                            multiseed, registry y callbacks
|   +-- evaluation/            suite, evaluacion unificada, escenarios
|   |                            y estadistica (IC95, Wilcoxon)
|   +-- production/            export ONNX, inferencia edge, data feed
|   |                            y bucle horario
|   +-- saas/                  plataforma Flask LeaLink (app, modulos,
|                                modelos, templates y tests propios)
+-- tests/                   Suite pytest transversal (Apendice D)
\end{lstlisting}

Esta separación es la que recorre, fase a fase, el diagrama de dependencias
de la Figura~\ref{fig:flujo_dependencias}.

La Figura~\ref{fig:flujo_dependencias} complementa el árbol de directorios con
el mapa completo de componentes y dependencias del pipeline del Residual SAC,
en seis fases: datos/ETL, entrenamiento, evaluación, exportación ONNX,
producción \textit{edge} y SaaS. Cada caja es un módulo real del repositorio y
las flechas distinguen el flujo de la acción, las dependencias de código y los
artefactos que pasan de una fase a la siguiente.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{img/flujo_dependencias.png}
  \caption{Componentes y dependencias del pipeline completo del Residual SAC
  (seis fases, de los datos crudos al SaaS). Los artefactos que cruzan fases
  (\texttt{dataset\_final.csv}, \texttt{best\_model.zip},
  \texttt{residual\_sac\_actor.onnx}) aparecen resaltados.}
  \label{fig:flujo_dependencias}
\end{figure}

\chapter{Suite de Tests Automatizados}
\label{app:tests}

Los 76 tests automatizados ejecutables con \texttt{pytest} se distribuyen en
once módulos: \texttt{test\_dataset.py} (12 tests del pipeline ETL),
\texttt{test\_energy\_env\_continuo.py} (16 tests del entorno y el simulador),
\texttt{test\_mpc.py} (7 tests del controlador MPC, incluida la factibilidad
del LP sobre 1.000 escenarios aleatorios),
\texttt{test\_heuristic.py} (8 tests de los invariantes del \textit{baseline}
heurístico: respeto de los límites de SoC y las eficiencias, y ausencia de
arbitraje con la red),
\texttt{test\_residual\_env.py} (10 tests de la arquitectura residual),
\texttt{test\_delta\_zero.py} (2 tests de la propiedad de suelo),
\texttt{regresion/test\_golden.py} (14 tests de regresión \textit{golden
master} que fijan el comportamiento del simulador y los entornos frente a
refactorizaciones), \texttt{test\_fisica\_4flujos.py} y
\texttt{test\_decode\_equivalencia.py} (equivalencia de la física de 4 flujos
y del decodificador discreto como fuente única),
\texttt{test\_onnx\_equivalence.py} (2 tests de equivalencia numérica entre el
actor PyTorch y el exportado a ONNX) y \texttt{test\_edge\_loop\_saas.py}
(3 tests de integración del bucle \textit{edge} con la API del SaaS). El
...
$+44{,}05$\,€/semana sobre el conjunto de evaluación (IC95
$[41{,}90;\,46{,}25]$, suite de validación, Sección~\ref{sub:resultados}),
equivalente a unos 2.300\,€/año para el conjunto de la comunidad (horquilla
del IC95: 2.185 a 2.412\,€/año). Representa el valor marginal real de la
batería (el desplazamiento temporal de energía que el controlador optimiza),
independiente del autoconsumo solar directo, que es atribuible a los paneles y
no al almacenamiento. Conviene matizar que esta cifra refleja el régimen de
precios del periodo 2021--2023, que incluye la crisis energética de 2022; en
un escenario de precios más moderados el beneficio anual sería menor.
...
beneficio marginal validado del controlador ($+44{,}05$\,€/semana,
...
$\times$ \SI{80}{kWh}) y un beneficio anual del orden de 2.300\,€, el periodo
de retorno simple sin ayudas se sitúa en torno a los nueve años (8,3 a 9,2
años según la horquilla del IC95), frente a una vida útil de la batería LFP de
15 a 20 años. Es, por tanto, una inversión de retorno positivo aunque ajustado
en ausencia de ayudas públicas.

\section{El Papel de las Subvenciones}

El marco de ayudas vigente en España modifica sustancialmente esta conclusión.
Los programas de incentivos a comunidades energéticas del IDAE (Programa
CE~IMPLEMENTA, Orden TED/764/2024 \citep{TED764_2024}) contemplan proyectos de
autoconsumo colectivo con almacenamiento, en los que las comunidades de
propietarios son elegibles, con intensidades de ayuda a fondo perdido de hasta
el 60\,\% para la generación renovable y el 30\,\% para el almacenamiento, lo
que en un proyecto mixto de autoconsumo con batería supone en torno al
40\,\% de la inversión total. A esto se suma la
deducción en el IRPF por obras de mejora de la eficiencia energética
(RD-ley 19/2021 \citep{RDL19_2021}, prorrogado) y bonificaciones municipales
en el IBI.

Aplicando una subvención del 40\,\%, la inversión neta desciende a unos
12.000\,€, lo que reduce el periodo de retorno a algo más de cinco años,
...
aunque ajustado sin ayudas públicas (en torno a nueve años), que el marco de
...
\chapter{Resultados Completos de la Suite de Validación}
\label{app:suite_completa}

Este anexo recoge las 31 versiones de la suite de validación y los cuatro
\textit{baselines}, evaluados bajo el protocolo unificado (50 semanas
\textit{held-out} $\times$ 10 semillas de ruido; IC95 por \textit{bootstrap};
$p$ del test de Wilcoxon pareado por semana y semilla de ruido frente al MPC realista;
dimensionado 42\,kWp\,/\,80\,kWh). Fuente: \texttt{resultados/resultados.csv},
generado por \texttt{src/evaluation/suite.py} a partir del registro
\texttt{config/experimentos.yaml}. Las versiones con una sola semilla de
entrenamiento son confirmaciones puntuales de ablación; el resto usa tres
semillas y despliega la mejor (Sección~\ref{sub:seleccion}).

\section{Baselines (sin aprendizaje)}

\begin{table}[H]
\centering\small
\begin{tabular}{lrcc}
\toprule
\textbf{Controlador} & \textbf{€/sem} & \textbf{IC95} & \textbf{$p$} \\
\midrule
G1 MPC oráculo & $51{,}45$ & $[44{,}07;\,60{,}29]$ & n/a \\
G2 MPC realista (referencia) & $43{,}39$ & $[41{,}24;\,45{,}57]$ & --- \\
G3 Heurístico & $30{,}49$ & $[28{,}14;\,32{,}91]$ & $4{,}6\cdot10^{-75}$ \\
G4 IDLE (sin batería) & $0{,}00$ & --- & n/a \\
\bottomrule
\end{tabular}
\end{table}

\section{Familia DQN (valor, discreta; 3 semillas)}

\begin{table}[H]
\centering\small
\begin{tabular}{llrcrc}
\toprule
\textbf{Versión} & \textbf{Config.\ clave} & \textbf{€/sem} & \textbf{IC95} &
\textbf{$\Delta$} & \textbf{$p$} \\
\midrule
DQN-1 & $n{=}1$, $\gamma{=}0{,}995$ & $39{,}36$ & $[37{,}21;\,41{,}62]$ & $-4{,}02$ & $5{,}0\cdot10^{-67}$ \\
DQN-2 & $n{=}8$, $\gamma{=}0{,}995$ & $37{,}24$ & $[35{,}02;\,39{,}57]$ & $-6{,}15$ & $5{,}0\cdot10^{-72}$ \\
DQN-3 & $n{=}8$, $\gamma{=}0{,}99$ & $38{,}34$ & $[36{,}17;\,40{,}62]$ & $-5{,}05$ & $1{,}0\cdot10^{-71}$ \\
DQN-4 & $n{=}24$ & $31{,}65$ & $[29{,}98;\,33{,}38]$ & $-11{,}74$ & $1{,}3\cdot10^{-83}$ \\
DQN-5 & $n{=}8$, lr $5\cdot10^{-4}$ & $37{,}16$ & $[35{,}06;\,39{,}33]$ & $-6{,}23$ & $5{,}5\cdot10^{-78}$ \\
DQN-6 & $n{=}4$ & $39{,}27$ & $[36{,}99;\,41{,}64]$ & $-4{,}11$ & $2{,}4\cdot10^{-66}$ \\
\bottomrule
\end{tabular}
\end{table}

\section{Familia PPO discreto (on-policy, discreta; 3 semillas)}
...
\textbf{$\Delta$} & \textbf{$p$} \\
\midrule
PPO-D1 & rollout 2048, ent 0{,}01 const. & $37{,}49$ & $[35{,}30;\,39{,}78]$ & $-5{,}89$ & $5{,}5\cdot10^{-73}$ \\
PPO-D2 & rollout 4096, ent 0{,}003 const. & $37{,}96$ & $[35{,}72;\,40{,}29]$ & $-5{,}43$ & $3{,}9\cdot10^{-69}$ \\
PPO-D3 & rollout 8192, ent con decaimiento & $38{,}91$ & $[36{,}66;\,41{,}22]$ & $-4{,}47$ & $4{,}2\cdot10^{-67}$ \\
PPO-D4 & rollout 8192, ent y lr con decaimiento & $39{,}35$ & $[37{,}09;\,41{,}70]$ & $-4{,}04$ & $2{,}4\cdot10^{-60}$ \\
PPO-D5 & rollout 8192, ent 0{,}01 const. & $37{,}14$ & $[34{,}96;\,39{,}40]$ & $-6{,}24$ & $5{,}2\cdot10^{-75}$ \\
\bottomrule
\end{tabular}
\end{table}

\section{Familia PPO continuo (on-policy, continua; 3 semillas)}
...
\textbf{$\Delta$} & \textbf{$p$} \\
\midrule
PPO-C1 & std$_0$ 0{,}5; ent y lr con decaimiento & $38{,}14$ & $[35{,}84;\,40{,}56]$ & $-5{,}24$ & $4{,}2\cdot10^{-41}$ \\
PPO-C2 & std$_0$ 0{,}3; ent y lr con decaimiento & $33{,}66$ & $[31{,}53;\,35{,}88]$ & $-9{,}73$ & $1{,}5\cdot10^{-80}$ \\
PPO-C3 & rollout 4096; ent y lr constantes & $38{,}31$ & $[36{,}08;\,40{,}66]$ & $-5{,}07$ & $6{,}5\cdot10^{-45}$ \\
\bottomrule
\end{tabular}
\end{table}

\section{SAC puro (continua, sin MPC; 3 semillas)}
...
\textbf{$\Delta$} & \textbf{$p$} \\
\midrule
F5 & SAC directo, sin MPC ni DAWN & $26{,}94$ & $[25{,}53;\,28{,}40]$ & $-16{,}44$ & $1{,}3\cdot10^{-83}$ \\
\bottomrule
\end{tabular}
\end{table}

\section{Familia Residual SAC (controlador propuesto)}

\begin{table}[H]
\centering\small
\begin{tabular}{llcrcrc}
\toprule
\textbf{Versión} & \textbf{Config.\ clave} & \textbf{Sem.} & \textbf{€/sem} &
\textbf{IC95} & \textbf{$\Delta$} & \textbf{$p$} \\
\midrule
SAC-A & aditiva, $\delta{=}0{,}05$, ent 0{,}005 & 3 & $42{,}47$ & $[40{,}28;\,44{,}73]$ & $-0{,}91$ & $1{,}8\cdot10^{-28}$ \\
SAC-B & mult., $\delta{=}0{,}30$, ent 0{,}002 & 3 & $42{,}31$ & $[40{,}20;\,44{,}48]$ & $-1{,}07$ & $7{,}4\cdot10^{-40}$ \\
\textbf{SAC-C} & mult., $\delta{=}0{,}15$, ent 0{,}01 & 3 & $\mathbf{44{,}05}$ & $[41{,}90;\,46{,}25]$ & $\mathbf{+0{,}66}$ & $1{,}5\cdot10^{-23}$ \\
F2-d05 & $\delta{=}0{,}05$ & 1 & $43{,}79$ & $[41{,}64;\,45{,}99]$ & $+0{,}41$ & $6{,}6\cdot10^{-55}$ \\
F2-d10 & $\delta{=}0{,}10$ & 1 & $44{,}01$ & $[41{,}86;\,46{,}22]$ & $+0{,}63$ & $2{,}8\cdot10^{-37}$ \\
F2-d20 & $\delta{=}0{,}20$ & 1 & $43{,}78$ & $[41{,}65;\,45{,}97]$ & $+0{,}40$ & $5{,}9\cdot10^{-9}$ \\
F2-d30 & $\delta{=}0{,}30$ & 1 & $42{,}43$ & $[40{,}35;\,44{,}57]$ & $-0{,}95$ & $1{,}7\cdot10^{-27}$ \\
F4-nodawn & sin \textit{warmup} DAWN & 1 & $44{,}04$ & $[41{,}90;\,46{,}23]$ & $+0{,}65$ & $3{,}1\cdot10^{-24}$ \\
F6-s05 & $\sigma_{\text{cons}}{=}5\,\%$ & 1 & $47{,}48$ & $[45{,}11;\,49{,}92]$ & $-0{,}79^{*}$ & $1{,}9\cdot10^{-68}$ \\
F6-s25 & $\sigma_{\text{cons}}{=}25\,\%$ & 1 & $40{,}02$ & $[38{,}11;\,41{,}99]$ & $+1{,}42^{*}$ & $8{,}6\cdot10^{-61}$ \\
\bottomrule
\multicolumn{7}{@{}p{\dimexpr\textwidth-2\tabcolsep}@{}}{\small Las versiones
F2/F4/F6 parten de la configuración de SAC-C y modifican solo el parámetro
indicado. No existe fila F6-s15: es exactamente SAC-C
($\sigma_{\text{cons}}{=}15\,\%$ es el valor estándar). $^{*}$En F6 la
$\Delta$ se calcula frente al MPC realista evaluado con el mismo nivel de
ruido que el agente.}
\end{tabular}
\end{table}

\section{Familia TD3 residual (3 semillas)}
...
TD3-3 & $\delta{=}0{,}15$, delay 3, lr $3\cdot10^{-4}$ & $43{,}57$ & $[41{,}45;\,45{,}72]$ & $+0{,}19$ & $0{,}07$ \\
TD3-4 & $\delta{=}0{,}10$, delay 2, ruido 0{,}2 & $44{,}17$ & $[42{,}02;\,46{,}37]$ & $+0{,}79$ & $1{,}5\cdot10^{-37}$ \\
\bottomrule
\end{tabular}
\end{table}

\section{Familia DDPG residual (3 semillas)}
...
\chapter{Reproducibilidad y Entornos de Entrenamiento}
\label{app:reproducibilidad}

Los 31 modelos de la suite se entrenaron distribuyendo las familias entre
cuatro máquinas, con entornos de software distintos. Este anexo documenta esos
entornos (extraídos del \texttt{system\_info.txt} que Stable-Baselines3
almacena dentro de cada artefacto \texttt{.zip}) y las condiciones bajo las
que los resultados son comparables.

\begin{table}[H]
\centering\small
\caption{Entornos de entrenamiento por familia.}
\label{tab:entornos}
\begin{tabular}{llll}
\toprule
\textbf{Familias} & \textbf{SO} & \textbf{Python} & \textbf{PyTorch} \\
\midrule
residual\_sac (10), sac\_puro, ppo\_continuo (3) & Windows 10 & 3.14.6 & 2.12.1+cpu \\
ppo discreto (5), ddpg\_residual (2) & Windows 10 & 3.12.7 & 2.12.1+cpu \\
dqn (6) & Linux (WSL2) & 3.14.4 & 2.12.1+cu130 \\
td3\_residual (4) & Linux (WSL2) & 3.12.3 & 2.12.1+cpu \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Coste de cómputo.} El entrenamiento completo de todos los modelos de
la suite consume aproximadamente 500 horas de cómputo en un ordenador de gama
media (CPU de escritorio, sin aceleración por GPU relevante para estos tamaños
de red). El grueso del coste corresponde a las familias residuales, que
resuelven un problema de programación lineal (el MPC base) en cada paso de
simulación; la distribución entre cuatro máquinas (Cuadro~\ref{tab:entornos})
respondió precisamente a la necesidad de paralelizar ese coste.

\textbf{Evaluación determinista y comparabilidad.} El protocolo de evaluación
es determinista: dado un modelo y una semilla de ruido, produce siempre el
mismo resultado, y los \textit{baselines} sin aprendizaje (MPC, heurístico,
IDLE) reproducen exactamente los mismos valores en todas las máquinas. Todas
las cifras publicadas se obtuvieron con el mismo protocolo y son, por tanto,
directamente comparables con independencia de dónde se entrenó cada modelo.

\textbf{Gestión de la varianza de entrenamiento.} El entrenamiento de RL
profundo no es reproducible bit a bit entre entornos distintos: el
\textit{build} de PyTorch, la versión de las librerías de álgebra y el número
de hilos alteran el orden de las operaciones en coma flotante, y el bucle de
aprendizaje amplifica esas diferencias microscópicas hasta producir modelos
distintos, un fenómeno documentado en la literatura \citep{Henderson2017}. En
la práctica el entorno actúa como una semilla adicional: introduce varianza,
no sesgo (ningún entorno favorece sistemáticamente a un algoritmo). Esa
varianza se gestiona en este trabajo igual que la de la semilla: tres semillas
de entrenamiento por versión, intervalos de confianza al 95\,\% por
\textit{bootstrap} y contraste de Wilcoxon pareado, de modo que ninguna
conclusión se apoya en una ejecución individual.

\textbf{Comparabilidad interna de la familia residual.} Todas las
comparaciones internas del controlador propuesto (formas aditiva y
multiplicativa, barrido $\delta$ de F2, ablación DAWN de F4 y sensibilidad al
pronóstico de F6) se entrenaron en un mismo entorno, por lo que no se ven
afectadas por esta heterogeneidad. La única comparación ajustada entre
entornos distintos es SAC-C frente a TD3-4, y se trata como tal en la
Sección~\ref{sub:resultados}: la diferencia entre ambos no es
estadísticamente significativa y la elección del controlador desplegado se
justifica por robustez y profundidad de caracterización, no por la media
puntual.
...
definitivo, su arquitectura se reentrenó y validó sobre
\SI{42}{kWp}/\SI{80}{kWh} dentro de la suite de validación bajo el
identificador SAC-C, cuyos resultados se reportan en la
Sección~\ref{sub:resultados} y en el Anexo~\ref{app:suite_completa}.
